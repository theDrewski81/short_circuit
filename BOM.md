...
|---|---|---|---|---|
| Drive gearmotor | **Pololu #5218** — 150:1 Micro Metal Gearmotor HPCB 12V with 12 CPR Encoder, **back connector** (N20-class, 12 mm) | 2 | $65 | Selected Session 03 after test-fitting; both sides identical. Back-connector variant confirmed (was an open question). Exact ratio 150.58:1 → 1807 counts/output-rev at 4× quadrature, which is **not** the operative figure: `gpiozero.RotaryEncoder` decodes 1×, so `COUNTS_PER_OUTPUT_REV` is **451.74** (3 cycles/motor-rev × 150.58), measured 450.6 at S10 (see `simulation/chassis/TUNING.md`). 9 mm × 3 mm D-shaft; gearbox cross section 10×12 mm. Run battery-direct at ~7.4 V (≈62% of 12 V rating). 6-pin JST SH encoder cables are supplied with the motor and need no line item of their own (confirmed with the parts in hand, 2026-08-25); the connector is recorded in `docs/HARDWARE_power_harness.md` so a replacement can be identified. An earlier note here saying they are sold separately was wrong. **Part number and price corrected 2026-08-25 (O6, session J5-S17):** the Pololu catalogue prices #5218 at **$32.45 each**, so $65 for the pair rather than the $46 previously carried. #5219 is the same 150:1 HPCB 12 V 12 CPR motor with a **side** connector at the same price; the chassis pocket carries a 3 × 11 mm relief cut for the **back** connector, so #5218 is the part. Both drive motors are #5218. There is no left/right variant, and any drawing or document showing #5218 on one side and #5219 on the other is wrong. |
...
| Treads | Printed TPU tank treads, Shore 90A | 2 | (filament) | Closed loop, 28 mm wide, 3.5 mm band, 364.42 mm inner path. **Centre-guide lug system (Session 04):** 29 lugs on the inner face at 12.566 mm pitch — the sprocket's own tooth pitch, so engagement accumulates no error around the loop — 3 mm tall, 8 mm at the tip flaring 45° to 14 mm at the root. Prints as a 28 mm-tall wall, loop plane on the bed, so the extrusions run along the tension path and the lug flare self-supports. **Printed as a circle, not in its running shape (2026-08-26):** r 58.0 mm inner, 123 mm across. A printed part is stress-free in the shape it was printed, so the print shape sets the curvature each element of the band is relaxed at, and every element travels the whole loop — flat on the two runs, 1/20 mm⁻¹ round a wheel. Built oval, an element is born at one extreme and swings the full 0.0500 mm⁻¹; built round, every element is born at 1/58 and the worst swing is 0.0328, taking outer-fibre bending strain through a tread rib from 8.75 % to 5.73 %. Mean curvature is pinned at 2π/path for any closed curve, so the circle is the best available shape and its radius is not a free choice. All 29 lugs and 24 chevrons keep their pitch, because offsetting a closed convex curve outward adds the same length whatever its shape. Secondary benefit: a ring is far less prone to warping or toppling on the bed than two 119 mm straight runs of thin tall wall. One per plate at 123 mm; the oval fitted two. **Herringbone treads (Session 04):** 24 chevrons on the ground face, 1.5 mm deep, 3.5 mm ribs at 25° from transverse, 15.7 mm pitch — 7.6 chevrons under the ground run at any moment. Each arm is built as five short pieces following the path, not one straight bar, so nothing floats clear of the band on the wrapped sections. Relieved *out of* the 3.5 mm envelope rather than added on top, so the tip radius, ride height and ground speed are unchanged and the continuous band becomes 2.0 mm — which nearly halves the outer-fibre bending strain around the 20 mm sprocket radius, helping flex-fatigue life rather than hurting it. Measured 36 g each, down from 50 g smooth. |
...
- ~~Drive motor part number and price contradiction (O6)~~ — closed 2026-08-25 against the Pololu catalogue: **#5218 for both motors, $32.45 each.** The 2026-06-26 entry's $32.45 was the right price attached to the wrong part number. The #5219 was bought deliberately as a fit test and does not fit the chassis cleanly. Three documents were left carrying the stale #5219, all outside that session's scope: `docs/HARDWARE_drive_bringup.md`, `docs/diagrams/bench_full_schematic.svg` and `simulation/chassis/TUNING.md`. All three were corrected 2026-08-25 in session J5-S18 (O12); in `TUNING.md` only the part numbers went, because the same line's left/right channel assignment was measured and is correct.
...
- ~~Mass ceiling breached (Session 04)~~ — **superseded; the ceiling is not breached.** The 1655 g reading predates the rework that followed it in the same session. `mechanical/preview/mass_budget.py` prints the current figure: **1542 g against the 1.6 kg ceiling**, 58 g of headroom, 1597 g at +25 % on the printed items, under the ceiling in both cases. The figure rose 11 g on 2026-08-26 with `23157ef`, which added the IMU carrier breadboard and grew the Pi-M shelf plate to take it; `ecb244f`, the circular track loop print, revised the same file's track entry the same evening and moved no mass, the loop's 36 g being a function of path length and thickness rather than of the shape it is printed in. The shells went to a 2.0 mm wall rather than the 1.6 mm this entry proposed, and two double-counted lines (the neck riser and the head nod ears) came out. See the Gate Condition Status section below. The identical paragraph was deleted from `phases/PHASE_00_HARDWARE.md` on 2026-08-23 as stale; this copy was missed and is corrected here (O13, session J5-S18).
...
**Met 2026-08-21.** Reopened 2026-08-20 for the chassis rework and closed the same week: drivetrain built and guarded, chassis reworked to carry it, every printed part measured from its built solid rather than estimated. Assembled mass 1542 g against the 1.6 kg ceiling — 58 g headroom, +25 % worst case 1597 g, also under; `mechanical/preview/mass_budget.py` is the source, and the figure moves whenever the geometry does. Shell walls went 2.4 -> 2.0 mm on torso and head to get there. Original close text follows.
