...
### Next session — initiating prompt (Phase 00, drivetrain)

> Johnny 5 — Phase 00, Session 04. Read CLAUDE.md, then `phases/PHASE_00_HARDWARE.md` (note the Session 03 handoff and decision log) and `BOM.md`. The chassis tub is printed and correct; the FreeCAD MCP is installed and working, so build the geometry and measure it rather than reasoning about it — `len(shape.Solids) != 1` and `a.common(b).Volume > 0` are the checks that earn their keep, and `validate.py` alone will not catch a defect. Build `mechanical/freecad/build_drivetrain.py` for the four parts that do not yet exist: drive sprocket (40 mm pitch dia, on a stepped ø6 hub bored 3 mm and set-screwed to the #5218 output shaft, running in an MR106ZZ in the rear wall, centred at x = ±75), front idler (40 mm, two 623ZZ in the hub on a full-width 3 mm rod), road wheels (2/side — resolve ø24-at-z-23.5 not reaching the track: either ø40 or axles down to z 15.5, and change `params.csv`, never the scripts), and the TPU track loop (28 mm wide, 3.5 mm band, 120 mm wheelbase, Shore 90A). Add build-time guards in the same style as `build_chassis.py`: the track must contact sprocket, idler and road wheels; the sprocket must clear the tub wall; nothing may foul the tread envelope. Then fix the rear caster, whose mount does not assemble — pivot bosses at x = ±16 bored along Y, arm eye at x = 0, and the wheel floating 6 mm above the floor with the arm hanging straight back; a trailing arm wants its pivot along X. Session config: Opus, Extended Thinking, High effort.
